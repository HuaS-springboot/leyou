...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 确定删除
            deleteRow(){
                axiosRequest(this.$URL.DELETE_MEMBER_LEVEL,{id:this.id},'POST').then((res)=>{
            ...
            // 保存添加会员等级
            saveEdit() {
                axiosRequest(this.$URL.SAVE_MEMBER_LEVEL,this.editForm,'POST').then((res)=>{
            ...
            //设置默认等级
            changeLever(row){
                axiosRequest(this.$URL.DEFAULT_MEMBER_LEVEL,{
                    id:row.id,
                    oldId:this.oldId
                },'post').then((res)=>{
                    this.oldId=row.id
