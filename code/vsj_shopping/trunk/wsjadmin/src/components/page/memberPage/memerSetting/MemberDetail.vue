  ...
  <div>
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-lx-calendar"></i> 会员详情
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
...
export default {
  ...
  created() {
    ...
    } else if (sessionStorage.getItem("memberDetailData")) {
      this.memberInfo = JSON.parse(sessionStorage.getItem("memberDetailData"));
      this.levelId = this.memberInfo.levelId;
      this.queryMemberDetail();
    }
    this.getLevel();
  ...
  methods: {
    ...
    queryMemberDetail() {
      axiosRequest(
        this.$URL.QUERY_MEMBER_DETAIL,
        { id: this.memberInfo.id },
        "POST"
      ).then(res => {
        this.memberDetail = res;
        console.log(this.memberDetail, 1333);
      });
    },
    //获取用户等级
    getLevel() {
