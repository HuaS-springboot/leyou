  ...
  <div>
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-lx-calendar"></i> 发起秒杀活动
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
      <div class="form-box addSeckillForm">
        <el-form ref="condition" :model="condition" label-width="160px" label-position="left">
          <el-form-item label="秒杀活动名称" class="seckillTitle">
            <el-input v-model="condition.activityName" placeholder="请输入秒杀活动名称" :disabled="seckillAbled"></el-input>
          </el-form-item>
          ...
          <el-table :data="condition.vsjActivitySeckills" border class="mt20">
            <el-table-column prop="attrJson2" label="规格" width="160"></el-table-column>
            <el-table-column prop="productStock" label="库存" width="160"></el-table-column>
            ...
            <el-table-column prop='activityPrice' label="秒杀价">
              <template scope="scope">
                <el-input
                  size="small"
            ...
            <el-table-column label="操作(删除即表示此规格不参加活动)">
              <template slot-scope="scope">
                <el-button class="typeIcon red" type="text" icon="el-icon-delete" @click="delSpec(scope.row)"></el-button>
              </template>
            </el-table-column>
          </el-table>
  ...
  methods: {
    ...
    getDetail(){
      if(sessionStorage.getItem('activeId')){   //编辑以及查看活动详情
        axiosRequest('activity/getActivityById',{
          ...
          this.condition.vsjActivitySeckills=res.vsjActivitySeckills
          this.condition.vsjActivitySeckills.forEach(item=>{
    ...
    goodsSpecs() {
      axiosRequest("activity/getSpecsList", {
        'id':this.condition.productId
          ...
          this.condition.vsjActivitySeckills=res
          this.condition.vsjActivitySeckills.forEach(item=>{
            item.attrJson2=item.attrJson.replace(/[&\|\\\*^%$#@,{}:'\-]/g,"")
            item.specsId=item.id
            // item.quotaNumber="1";
            // item.productNumber='300';
    ...
    delSpec(value){
      if(sessionStorage.getItem('activeId')){
        axiosRequest("activity/delActivitySpecs",{
          id:value.id,
          activityType:'1'
        },'post').then((res)=>{
          let index=this.condition.vsjActivitySeckills.indexOf(value)
          this.condition.vsjActivitySeckills.splice(index,1)
        })
      }else{
