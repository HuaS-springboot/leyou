    ...
    <div class="table">
        ...
        <div class="container">
            ...
            <div class="handle-box">
                <el-input v-model="conditon.title" placeholder="请输入要查询的商品名称" class="handle-input mr10 mb10" ></el-input>
                <el-input v-model="conditon.priceMin" placeholder="价格区间" class="handle-inputSmall mr5 mb10" ></el-input>—
                <el-input v-model="conditon.priceMax" placeholder="价格区间" class="handle-inputSmall mr10 mb10" ></el-input>
        ...
        <el-dialog title="编辑库存" :visible.sync="stockVisible" width="761px" center>
            <el-table :data="guigeData" border  class="guigeEdit">
                ...
                <el-table-column prop="saleNum" label="销量" width="150px"> 
                    <template scope="scope">
                        <el-input size="small" v-show="scope.row.saleNum" v-model="scope.row.saleNum" placeholder="请输入属性值"></el-input>
                    </template>
                </el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getGoodsList(){
                //获取商品列表
                axiosRequest('product/getProductList',this.conditon,'post').then((res)=>{
            ...
            typeChange(event){
                console.log(event)
                if(event.length==1){
                    this.conditon.oneCateId=event[0]
                }else if(event.length==2){
                    this.conditon.twoCateId=event[1]
                }else if(event.length==3){
                    this.conditon.threeCateId=event[2]
                }else if(event.length==0){
                    this.conditon.oneCateId=""
                    this.conditon.twoCateId=""
                    this.conditon.threeCateId=""
                }
                this.conditon.pageNum = 1;
                this.getGoodsList()
            },
            //商品上架状态查询
            stateChange(event){
                this.conditon.status=this.value
                this.conditon.pageNum = 1;
                this.getGoodsList()
            },
            // 分页导航
